    ...
    if passed1 == True:
        ...
        try:
            ...
            if (B - C) < (D - C) and (B - C) < (D - E) and (F - E) < (D - E) and (F - E) < (
                    D - C) and D > B and D > F and C < B and E < F and local_min[-3] < local_max[-3] < local_min[-2] < local_max[-2] < local_min[-1] < local_max[-1]:
                fig = plt.figure(figsize=(10, 7))
                plt.plot([], [], ' ')
                plt.title(f'IETE : debug', fontweight="bold", color='black')
